...
class _ColisListPageState extends State<ColisListPage> {
  ...
  final List<String> modeEnvoie = ['Express', 'Maritime', 'Batterie'];
  ...
  Future<void> fetchDataWithFilters() async {
    ...
    try {
      ...
      print('Error: $error');
      setState(() {
        isLoading = false; // Indiquer que le chargement a échoué
      });
    }
  }
  ...
  Future<void> fetchData() async {
    ...
    try {
      final response = await http.get(
          Uri.parse('https://s-tradingmadagasikara.com/getColis.php'));
        ...
        if (jsonData['success'] == 1) {
          ...
          setState(() {
            colis = colisData.isNotEmpty
                ? ColisModel.fromJson(colisData.first)
                : null;
            allColisList = colisData.map((json) => ColisModel.fromJson(json)).toList();
            colisList = List.from(allColisList); // Copiez les données dans la liste non filtrée
            isLoading = false;
          });
  ...
  Future<void> scanBarcode() async {
    ...
    ColisModel? colis;
    for (ColisModel colis in colisList) {
      if (colis.tracking == barcode) {
        colis = colis;
        break; // Sortez de la boucle si le colis est trouvé.
      }
    }
    if (colis != null) {
      Navigator.push(
      ...
      // Aucun colis correspondant n'a été trouvé.
              ...
              children: [
                ...
                TextFormField(
                  ...
                  controller: TextEditingController(
                      text: colis?.poids?.toString() ?? ''),
                ),
                TextFormField(
                  decoration: InputDecoration(labelText: "Volume en m3"),
                  keyboardType: TextInputType.number,
                  controller: TextEditingController(
                      text: colis?.volume?.toString() ?? ''),
                ),
                TextFormField(
                  decoration: InputDecoration(labelText: "Facture en Ar"),
                  keyboardType: TextInputType.number,
                  controller: TextEditingController(
                      text: colis?.facture?.toString() ?? ''),
                ),
              ...
              IconButton(
                ...
                onPressed: () {
                  ...
                  setState(() {
                    ...
                    colisList = List.from(allColisList); // Réinitialisez la liste avec toutes les données

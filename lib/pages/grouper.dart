...
class _GrouperState extends State<Grouper> {
          ...
          return (snapshot.connectionState == ConnectionState.waiting)
            ...
            child: CircularProgressIndicator(),
              ...
              itemBuilder: (context, index) {
                ...
                if (tracking.isEmpty || cartonDocs[index]['tracking']
                    .toString()
                    .toLowerCase()
                                  ...
                                  showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        ...
                                        content: const Text("Êtes-vous sûr de vouloir supprimer ce carton?"),
                                        actions: [
                                          TextButton(
  ...
  Future<void> updateEtatAndDate(String cartonId, String etat) async {
    try {
      ...
      await FirebaseFirestore.instance.collection('cartons').doc(cartonId).update({
        'etat': etat,
      });

      // Obtenez les colis membres du carton
      DocumentSnapshot cartonSnapshot = await FirebaseFirestore.instance.collection('cartons').doc(cartonId).get();
      List<dynamic> colistracking = cartonSnapshot.get('trackingColis');

      // Mettez à jour les colis membres du carton
      for (var tracking in colistracking) {
        QuerySnapshot colisSnapshot = await FirebaseFirestore.instance.collection('colisDetails').where('tracking', isEqualTo: tracking).get();
